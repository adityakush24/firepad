...
describe("Parse HTML Tests", function () {
  ...
  it("Handle newlines and extra spaces in html.", function () {
    parseTest("Foo\nBar\nBaz    Blah", [Line([Text("Foo Bar Baz Blah")], lf)])
  })

  function styleTest(style, textFormatting) {
    var html = '<div style="' + style + '">Test</div>'
    parseTest(html, [Line([Text("Test", textFormatting)], lf)])
  }
  ...
  it("Supported font tags", function () {
    ...
    fontTest(
      'size="8" color="blue" face="impact"',
      tf.fontSize(8).color("blue").font("impact")
    )
  })
  ...
  it("Nested divs", function () {
    parseTest(
      '<div style="color: green"><div style="font-weight: bold">Foo</div>bar</div><div><div style="background-color: blue">Baz</div></div>',
      [
  ...
  it("Spans with styles", function () {
    parseTest(
      '<span style="font-weight: bold">Foo<span style="color: green">bar</span>baz</span><span style="background-color: blue">Lorem ipsum</span>',
      [
        Line(
          [
            Text("Foo", tf.bold(true)),
            Text("bar", tf.bold(true).color("green")),
            Text("baz", tf.bold(true)),
            Text("Lorem ipsum", tf.backgroundColor("blue")),
          ],
          lf
        ),
      ]
  ...
  function entityText(entityType, info) {
    var formatting = new Formatting(new Entity(entityType, info).toAttributes())
  ...
  it("Images", function () {
    ...
    parseTest('Foo<img src="http://www.google.com/favicon.ico">Foo', [
      Line(
        [t, entityText("img", { src: "http://www.google.com/favicon.ico" }), t],
        lf
      ),
    ])
  })
  ...
  it("Complex list (1)", function () {
    var t = Text("Foo", tf)
    parseTest(
      "<ol><li>Foo<ol><li>Foo</li><li>Foo</li></ol></li><li>Foo</li></ol>",
      [
        Line([t], lf.indent(1).listItem(LIST_TYPE.ORDERED)),
  ...
  it("Todo list support", function () {
    var t = Text("Foo", tf)
    parseTest(
      '<ul class="firepad-todo"><li>Foo</li><li class="firepad-checked">Foo</li></ul>',
      [
  ...
  it("<center> support (1)", function () {
    parseTest("<center>foo</center>", [Line([Text("foo")], lf.align("center"))])
  })
  ...
  function parseTest(html, expLines) {
    ...
    if (i < actLines.length) {
      throw new Error(
        "Unexpected extra line " + i + ":" + dumpLine(actLines[i])
      )
    }
