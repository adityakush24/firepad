    ...
    helpers.randomTest(n, function () {
      ...
      var list = new AnnotationList(function (oldNodes, newNodes) {
        ...
        for (var i = 0; i < oldNodes.length; i++) {
          expect(
            oldNodes[i].pos + oldNodes[i].length <=
              attributesFromEvents.length + deletedChars
          ).toBe(true)
          attributesFromEvents.splice(
            oldNodes[i].pos - deletedChars,
            oldNodes[i].length
          )
      ...
      for (var i = 0; i < attributes.length; i++) {
        list.insertAnnotatedSpan(
          new Span(i, 1),
          new AttributesAnnotation(attributes[i])
        )
      }
      ...
      for (i = 0; i < o.ops.length; i++) {
        ...
        if (op.isInsert()) {
          list.insertAnnotatedSpan(
            new Span(pos, op.text.length),
            new AttributesAnnotation(op.attributes)
          )
          pos += op.text.length
        } else if (op.isRetain()) {
          list.updateSpan(
            new Span(pos, op.chars),
            makeUpdateCallback(op.attributes)
          )
          pos += op.chars
        } else if (op.isDelete()) {
