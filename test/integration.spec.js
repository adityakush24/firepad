...
describe("Integration tests", function () {
  ...
  it("Uses defaultText to initialize the pad properly", function (done) {
    ...
    firepad.on("ready", function () {
      ...
      var waitForSync = new Promise(function (resolve) {
        firepad.on("synced", function (isSync) {
          if (isSync) resolve()
        })
      })
      waitForSync.then(function () {
        ...
        firepad2.on("ready", function () {
          if (firepad2.getText() == text2) {
              ...
              new Error(
                "Second Firepad got neither default nor edited text: " +
                  JSON.stringify(firepad2.getText())
              )
            )
          }
        })
  ...
  it("Performs Firepad.dispose", function (done) {
    ...
    firepad.on("ready", function () {
      ...
      expect(function () {
        firepad.setText("I'm a zombie.  Braaaains...")
      }).toThrow()
      expect(function () {
        firepad.getHtml()
      }).toThrow()
      expect(function () {
        firepad.setHtml("<p>I'm a zombie.  Braaaains...</p>")
      }).toThrow()
      done()
    })
  })
  ...
  it("Performs headless get/set plaintext & dispose", function (done) {
    ...
    firepadHeadless.setText(text, function () {
      firepadHeadless.getText(function (headlessText) {
        ...
        expect(function () {
          firepadHeadless.getText(function () {})
        }).toThrow()
        expect(function () {
          firepadHeadless.setText("I'm a zombie.  Braaaains...")
        }).toThrow()
        done()
      })
    })
  ...
  it("Performs headless get/set html & dispose", function (done) {
    ...
    firepadHeadless.setHtml(html, function () {
      firepadHeadless.getHtml(function (headlessHtml) {
        ...
        expect(firepadHeadless.firebaseAdapter_.firebaseCallbacks_).toEqual([])
        expect(function () {
          firepadHeadless.getHtml(function () {})
        }).toThrow()
        expect(function () {
          firepadHeadless.setHtml("<p>I'm a zombie.  Braaaains...</p>")
        }).toThrow()
        done()
      })
    })
  ...
  it("Ace editor", function (done) {
    ...
    var editor = ace.edit(
      hiddenDiv().appendChild(document.createElement("div"))
    )

    var text =
      "// JavaScript in Firepad!\nfunction log(message) {\n  console.log(message);\n}"
