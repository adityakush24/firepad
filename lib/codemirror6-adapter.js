...
var CodeMirror6Adapter = function () {
  ...
  function CodeMirror6Adapter(cmIstance) {
    const { StateEffect, EditorView } = window.StaticCodeMirror
      ...
      effects: StateEffect.appendConfig.of(
        ...
        EditorView.updateListener.of((v) => {
          if (v.docChanged && !(
            ...
            let index = 0
            let change_op  = new firepad.TextOperation()
            let inverse_op = new firepad.TextOperation()
...
CodeMirror6Adapter.prototype.applyOperation = function applyOperation(operation) {
  const {Annotation} = window['StaticCodeMirror']
...
firepad.CodeMirror6Adapter = CodeMirror6Adapter;
