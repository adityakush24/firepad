...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Firepad = factory());
  ...
  const FirebaseAdapter = (function (global) {
  ...
  const getDocument = () => fDocument || window.document;

  const setDocument = (document) => fDocument = document;
  ...
  const ParseHtml = (function () {
    ...
    function parseNode(node, state, output) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (entity) {
          output.currentLine.push(new Text(
   ...
   */
  const textPiecesToInserts = function(atNewLine, textPieces) {
    var inserts = [];
    ...
    function insertLine(line, withNewline) {
      ...
      if (atNewLine) {
        insert(sentinelConstants.LINE_SENTINEL_CHARACTER, line.formatting.attributes);
      }
  ...
  const RichTextCodeMirror = (function () {
    ...
    var LineSentinelCharacter   = sentinelConstants.LINE_SENTINEL_CHARACTER;
    var EntitySentinelCharacter = sentinelConstants.ENTITY_SENTINEL_CHARACTER;
    ...
    RichTextCodeMirror.prototype.insertEntityAt = function(index, type, info, origin) {
      this.codeMirror;
      this.insertEntity_(index, new Entity(type, info), origin);
    };
    ...
    RichTextCodeMirror.prototype.onCodeMirrorChange_ = function(cm, cmChanges) {
      ...
      for (var i = 0; i < changes.length; i++) {
        var change = changes[i];
        var start = change.start; change.end; var text = change.text, removed = change.removed, origin = change.origin;
    ...
    RichTextCodeMirror.prototype.markLineSentinelCharactersForChanges_ = function(changes) {
      ...
      for (var i = 0; i < changes.length; i++) {
        var change = changes[i];
        var line = change.from.line; change.from.ch;
  ...
  var ACEAdapter = (function() {
    ...
    ACEAdapter.prototype.operationFromACEChange = function(change) {
      var delete_op, delta, insert_op, ref, restLength, start, text;
      if (change.data) {
        delta = change.data;
        if ((ref = delta.action) === 'insertLines' || ref === 'removeLines') {
          text = delta.lines.join('\n') + '\n';
          delta.action.replace('Lines', '');
        } else {
          text = delta.text.replace(this.aceDoc.getNewLineCharacter(), '\n');
          delta.action.replace('Text', '');
        }
      ...
      insert_op = new TextOperation().retain(start).insert(text).retain(restLength);
      delete_op = new TextOperation().retain(start)["delete"](text).retain(restLength);
      if (change.action === 'remove') {
  ...
  */
   ...
   * @param {any} clientID - ID number of the Remote Client
   */
  CodeMirror6Adapter.prototype.setOtherCursor = function setOtherCursor(cursor, color, clientID) {
  };
   ...
   * @param {function[]} callbacks - Set of callback functions
   */
  CodeMirror6Adapter.prototype.registerCallbacks = function registerCallbacks(callbacks) {
    this.callbacks = Object.assign({}, this.callbacks, callbacks);
  };
  ...
  CodeMirror6Adapter.prototype.registerUndo = function registerUndo(callback) {
    if (typeof callback === 'function') {
      ...
      throw new Error('CodeMirror6Adapter: registerUndo method expects a '
        + 'callback function in parameter');
    }
  };
  ...
  CodeMirror6Adapter.prototype.registerRedo = function registerRedo(callback) {
    if (typeof callback === 'function') {
      ...
      throw new Error('CodeMirror6Adapter: registerRedo method expects a '
        + 'callback function in parameter');
    }
  };
   ...
   * @param {Operation} operation - OT.js Operation Object
   */
  CodeMirror6Adapter.prototype.applyOperation = function applyOperation(operation) {
   ...
   * @param {Operation} operation - OT.js Operation Object
   */
  CodeMirror6Adapter.prototype.invertOperation = function invertOperation(operation) {
    operation.invert(this.getValue());
  };
  ...
  const Firepad = (function() {
    ...
    function Firepad(ref, place, options) {
      ...
      if (!CodeMirror && !ace && !global.monaco && !(place.type === 'CodeMirror6')) {
    ...
    Firepad.fromCodeMirror6 = (ref, editor, ...e) => Firepad(ref, {
      type: 'CodeMirror6',
      editor,
    }, ...e);
